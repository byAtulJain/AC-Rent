...
class _EnquiryPageState extends State<EnquiryPage> {
  ...
  List<Map<String, dynamic>> rentPeriods = [];
  ...
  void fetchRentPeriods() async {
    try {
      final snapshot =
          await FirebaseFirestore.instance.collection('Rent Period').get();
      setState(() {
        rentPeriods = snapshot.docs
            .map((doc) => {'id': doc.id, 'period': doc['period']})
            .toList();
        isLoadingRentPeriods = false;
      });
    } catch (e) {
      print('Error fetching rent periods: $e');
      setState(() {
        isLoadingRentPeriods = false;
      });
    }
      ...
      setState(() {
        firebaseProducts = snapshot.docs.map((doc) {
          ...
          return {
            ...
            'price': 'Price: Rs. ${data['price']}/month',
          };
        }).toList();
  ...
  Future<void> submitEnquiry() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('Rent Enquiry').add({
        ...
        'selectedDuration': rentPeriods
            .firstWhere((period) => period['id'] == selectedDuration)['period'],
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildDropdownField(
                        ...
                        items: isLoadingRentPeriods
                            ? []
                            : rentPeriods.map((rentPeriod) {
                                return DropdownMenuItem<String>(
                                  value: rentPeriod['id'],
                                  child: Text(rentPeriod['period']),
                                );
                              }).toList(),
